...
export default function RestaurantCard({rest}:{rest:Restaurant}){
    ...
    return (
       ...
       <picture className="grid gap-8  justify-items-center place-items-center h-full" key={rest.id}>
               
            <Link className="" href={`/${rest.id}`}>
                <h2 className="text-3xl border-b border-b-transparent group-hover:text-orange-500  group-hover:border-b-orange-500 transition-all">{rest.name}</h2>
            </Link>
            <div className="grid  h-40 bg-violet-800 bg-opacity-5 justify-self-center place-content-center" >
                <img className="h-full transition-all group-hover:rounded-lg " src={rest.image} alt="Imagen del Restaurant" />
            </div>
